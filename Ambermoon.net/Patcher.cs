...
internal class Patcher(IRenderView renderView, string patcherPath, TextureAtlasManager textureAtlasManager)
{
    ...
    readonly List<UI.UIText> texts = [];
    Render.Cursor cursor = null;
    readonly static Size windowSize = new(16, 8);
    readonly static Rect windowArea = new
    (
    ...
    readonly static Rect clientArea = new(windowArea.X + 16, windowArea.Y + 16, windowArea.Width - 32, windowArea.Height - 32);
    GameLanguage language = GameLanguage.English;
    readonly List<UI.Button> buttons = [];
    readonly List<IColoredRect> buttonBackgrounds = [];
    readonly List<IColoredRect> filledAreas = [];
    Action clickHandler = null;
    long ticks = 0;
    readonly object uiLock = new();
    bool finished = false;
    readonly Queue<Action> uiChanges = [];
