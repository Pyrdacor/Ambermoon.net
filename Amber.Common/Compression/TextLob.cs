        ...
        public static byte[] CompressData(byte[] data)
        {
            ...
            void CheckDataValidity(int index)
            {
                if (data[index] > 0 && data[index] < 32)
                    throw new AmberException(ExceptionScope.Data, $"Unsupported text data at index {index}.");
            }

            int skipBytes = data.ToList().FindLastIndex(b => b > 0 && b < 32) + 1;

            if (skipBytes > 255)
                throw new AmberException(ExceptionScope.Data, "Data can't be compressed with text lob.");
