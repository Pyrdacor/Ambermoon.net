...
namespace Ambermoon.Data.Legacy.Characters
{
    public class CharacterManager : ICharacterManager
    {
        ...
        public CharacterManager(IGameData gameData)
        {
            ...
            foreach (var npcFile in gameData.Files["NPC_char.amb"].Files)
                npcs.Add((uint)npcFile.Key, NPC.Load(npcReader, npcFile.Value, gameData.Files["NPC_texts.amb"].Files[npcFile.Key]));
            foreach (var monsterFile in gameData.Files["Monster_char_data.amb"].Files)
                monsters.Add((uint)monsterFile.Key, Monster.Load(monsterReader, monsterFile.Value));
