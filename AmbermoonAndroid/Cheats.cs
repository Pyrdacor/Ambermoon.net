    ...
    class Cheats
    {
        ...
        static void Netsrak(Game game, string[] args)
        {
            ...
            foreach (var partyMember in game.PartyMembers)
            {
                ...
                foreach (var attribute in Ambermoon.Enum.GetValues<Ambermoon.Data.Attribute>())
                {
                    if (attribute < Ambermoon.Data.Attribute.Age)
                        partyMember.Attributes[attribute].CurrentValue = partyMember.Attributes[attribute].MaxValue;
                }

                foreach (var ability in Ambermoon.Enum.GetValues<Ability>())
                {
        ...
        static void Teleport(Game game, string[] args)
        {
            ...
            if (args.Length >= 3)
            {
                var world = Ambermoon.Enum.GetValues<World>().Cast<World?>().FirstOrDefault(w =>
        ...
        static void Fly(Game game, string[] args)
        {
            ...
            if (game.ActivateTransport(Ambermoon.Data.Enumerations.TravelType.Fly))
            {
