...
internal class ItemGrid
{
    ...
    public static readonly Size SlotSize = new(SlotWidth, SlotHeight);
        ...
        ScrollbarType? scrollbarType = null, bool showPrice = false, Func<uint>? availableGoldProvider = null)
    {
        ...
        scrollbar = slotsPerScroll == 0 ? null :
            new Scrollbar(game, layout, scrollbarType ?? ScrollbarType.SmallVertical, scrollbarArea ?? new(),
            scrollbarSize?.Width ?? 0, scrollbarSize?.Height ?? 0, (numTotalSlots - slotsPerPage) / slotsPerScroll);
        if (scrollbar != null)
        {
    ...
    public static ItemGrid CreateInventory(GameCore game, Layout layout, int partyMemberIndex, IGameRenderView renderView,
        ...
        Action<ItemGrid, int, ItemSlot> equipHandler, Action<ItemGrid, int, ItemSlot> useHandler)
    {
        ...
        grid.dropSlotProvider = (position, broken, _) =>
        {
            ...
            if (slot != null)
            {
                ...
                if (itemSlot?.Flags.HasFlag(ItemSlotFlags.Locked) ?? false)
                {
    ...
    public static ItemGrid CreateEquipment(GameCore game, Layout layout, int partyMemberIndex, IGameRenderView renderView,
    ...
    public static ItemGrid Create(GameCore game, Layout layout, IGameRenderView renderView, IItemManager itemManager,
    ...
    {
        ItemAnimation.Play(game, renderView, ItemAnimation.Type.Shake, Position.Zero, finishAction, TimeSpan.FromMilliseconds(50),
            Position.Zero, items[SlotFromItemSlot(itemSlot)]);
    }
}
