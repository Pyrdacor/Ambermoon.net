...
namespace Ambermoon.Renderer
{
    internal class TextureShader : ColorShader
    {
        ...
        protected static string[] TextureFragmentShader(State state) => new string[]
        {
            ...
            $"    {{",
            $"        // Color key is only available for UI palettes 49 and 51.",
            $"        float colorKey = abs(palIndex - 49.0f) < 0.5f || abs(palIndex - 51.0f) < 0.5f ? {DefaultColorKeyName} : 0.0f;",
            $"        ",
            $"        if (abs(colorIndex - colorKey) < 0.5f)",
            $"            discard;",
            $"        else",
