...
namespace Ambermoon.Renderer
{
    internal class TextShader : TextureShader
    {
        ...
        static string[] TextFragmentShader(State state) => new string[]
        {
            ...
            $"            discard;",
            $"        else",
            $"            {DefaultFragmentOutColorName} = texture({DefaultPaletteName}, vec2((textColIndex + 0.5f) / 32.0f, (palIndex + 0.5f) / {DefaultPaletteCountName}));",
            $"    }}",
            $"    else",
            $"    {{",
            ...
            $"        else",
            $"        {{",
            $"            vec4 textColor = texture({DefaultPaletteName}, vec2((textColIndex + 0.5f) / 32.0f, (palIndex + 0.5f) / {DefaultPaletteCountName}));",
