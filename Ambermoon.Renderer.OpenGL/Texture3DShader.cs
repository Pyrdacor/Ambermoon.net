...
namespace Ambermoon.Renderer
{
    internal class Texture3DShader : ColorShader
    {
        ...
        // The palette has a size of 32x70 pixels.
        ...
        static string[] Texture3DFragmentShader(State state) => new string[]
        {
            ...
            $"    vec4 pixelColor = texture({DefaultPaletteName}, vec2((colorIndex + 0.5f) / 32.0f, (palIndex + 0.5f) / 70.0f));",
            $"    ",
            $"    if (alphaEnabled > 0.5f && (colorIndex < 0.5f || pixelColor.a < 0.5f))",
            $"        discard;",
            $"    else",
            $"        {DefaultFragmentOutColorName} = vec4({DefaultLightName} * pixelColor.rg, min(1.0f, 0.25f + {DefaultLightName}) * pixelColor.b, 1.0f);",
            $"}}"
        };
