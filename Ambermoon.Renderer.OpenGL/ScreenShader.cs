...
namespace Ambermoon.Renderer
{
    ...
    internal class ScreenShader
    {
        ...
        static string[] ScreenFragmentShader(State state) => new string[]
        {
            ...
            $"    ",
            $"    float leftFactor = min(1.0f, 1.0f - offset.x);",
            $"    float rightFactor = min(1.0f, 0.0f + offset.x);",
            $"    float topFactor = min(1.0f, 1.0f - offset.y);",
            $"    float bottomFactor = min(1.0f, 0.0f + offset.y);",
            $"    ",
            ...
            $"    {{",
            $"        const vec3 add = gray(color11) < 0.1f ? vec3(0.0f) : vec3(0.055f);",
            ...
            $"    {{",
            $"        const vec3 add = gray(color11) < 0.1f ? vec3(0.0f) : vec3(0.06f);",
            $"        if (twoDim < 0.5f) a = 0.2f;",
            $"        else color11 *= 0.85f;",
            $"        color11.rgb += add;",
            $"    }}",
            ...
            $"            color11.rgb += add;",
            $"            if (twoDim < 0.5f) {{ a = 0.075f; }}",
            $"            else {{ color11.a *= 0.925f; }}",
            $"        }}",
            $"        else if (m < 1.5f)",
            $"        {{",
            ...
            $"        {{",
            $"            if (twoDim < 0.5f) {{ a = 0.075f; }}",
            $"            else {{ color11.a *= 0.925f; color11.rgb *= vec3(0.75f, 0.75f, 0.75f); }}",
            $"        }}",
            $"    }}",
            $"    ",
