...
namespace Ambermoon.Renderer
{
    ...
    internal class ScreenShader
    {
        ...
        static string[] ScreenFragmentShader(State state) => new string[]
        {
            ...
            $"    return 1.0f - min(1.0f, 3.5f * normFactor * sqrt(yuvDist.r * yuvDist.r + yuvDist.g * yuvDist.g + yuvDist.b * yuvDist.b));",
            $"}}",
            $"",
            ...
            $"        ",
            $"        const float smoothFactor = 0.675f;",
            ...
            $"        color00 = mix(color11, color00, d00 * 0.5f * (leftFactor + topFactor));",
            $"        color10 = mix(color11, color10, d10 * topFactor);",
            $"        color20 = mix(color11, color20, d20 * 0.5f * (rightFactor + topFactor));",
            $"        color01 = mix(color11, color01, d01 * leftFactor);",
            $"        color21 = mix(color11, color21, d21 * rightFactor);",
            $"        color02 = mix(color11, color02, d02 * 0.5f * (leftFactor + bottomFactor));",
            $"        color12 = mix(color11, color12, d12 * bottomFactor);",
            $"        color22 = mix(color11, color22, d22 * 0.5f * (rightFactor + bottomFactor));",
            $"        ",
            ...
            $"    ",
            $"    float twoDim = a;",
            $"    if ({DefaultSecondaryModeName} > 0.5f && {DefaultSecondaryModeName} < 2.5f && mod(round(gl_FragCoord.y - 0.5f), 2) > 0.5f)",
            $"    {{",
            $"        const vec3 add = gray(color) < 0.025f ? vec3(0.0f) : vec3(-0.035f);",
            $"        if (twoDim < 0.5f) color.a = 0.125f;",
            $"        else color.rgb += add;",
            $"    }}",
            $"    else if ({DefaultSecondaryModeName} > 1.5f && {DefaultSecondaryModeName} < 2.5f && mod(round(gl_FragCoord.x - 0.5f), 2) > 0.5f)",
            $"    {{",
            $"        const vec3 add = gray(color) < 0.025f ? vec3(0.0f) : vec3(-0.035f);",
            $"        if (twoDim < 0.5f) color.a = 0.125f;",
            $"        else color.rgb += add;",
            ...
            $"        const float b = 1.0f;",
            $"        const float d = 0.9f;",
            $"        const vec3 add = gray(color) < 0.025f ? vec3(0.0f) : vec3(0.075f);",
            $"        float m = mod(round(gl_FragCoord.y - 0.5f), 5);",
            $"        if (m < 0.5f)",
            $"        {{",
